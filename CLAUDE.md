...
**Idle Ore Generators** — a Minecraft **Bedrock** addon (Behavior Pack + Resource Pack) written in TypeScript against `@minecraft/server` 2.x. Placeable "generator" blocks passively produce vanilla resources on **real-world time**, including while the player/world is offline. Published on CurseForge; requires Minecraft Bedrock **1.21.130+**, no experimental features.
...
Build tooling is `just-scripts` via `@minecraft/core-build-tasks`. **Requires a `.env` file** at repo root defining `PROJECT_NAME=idleoregen` (read by `just.config.ts` via `getOrThrowFromProcess`) — the build throws without it.
...
- Localization lives in `resource_packs/idleoregen/texts/{en_US,es_ES}.lang`; both languages must be kept in sync.
